...
ln -sf $0 .$0.cpp && c++ -Wl,-rpath=$HOME/usr/lib$CLUSTER -std=c++0x -D_XYZ_TEST_HPP -I$HOME/usr/include -L$HOME/usr/lib$CLUSTER -laspell -I$HOME/prj `pkg-config --libs gsl` -lboost_system -lboost_regex -lboost_filesystem .$0.cpp -o .$0.x -Wall && ./.$0.x $@; exit
#endif
#ifndef XYZ_HPP_
#define XYZ_HPP_
...
namespace xyz{
	...
	class atom : public species, public boost::array<double, 3>{
		protected:
		friend class frame;
		quantity<nonsi::angstrom_unit>& operator[](size_t i){
			return const_cast<quantity<nonsi::angstrom_unit>&>(static_cast<const atom&>(*this).operator[](i)); //to avoid code duplication
		}
		public:
	...
	class frame : public std::vector<atom>, public boost::optional<crystal>{
		...
		frame& scale(double const& s){ //scale atom positions (and cell if any)
			for(unsigned i=0; i!=size(); ++i){
				for(unsigned j=0; j<3; ++j){
					std::vector<atom>::operator[](i)[j]*=s;
					//((std::vector<atom>&)(*this)[i])[j]*=s;
				}
			}
			if((boost::optional<crystal>&)(*this)){
				for(unsigned j=0; j!=3; ++j){
					for(unsigned i=0; i!=3; ++i){
						(*(boost::optional<crystal>&)(*this))[j][i]*=s;
					}
				}
			}
			return *this;
		}
		void save(boost::filesystem::path const& p) const{
			ofstream ofs(p);
			ofs << (*this);
			return;
		}
		private:
		friend ofstream& operator<<(ofstream& os, frame const& f){
		...
		friend ifstream& operator>>(ifstream& is, frame& f){
			...
			//is.ignore(); no, this will eat the fist atom name if there is no space
			f.reset();
			f.clear();
			//f.reserve(natoms);
