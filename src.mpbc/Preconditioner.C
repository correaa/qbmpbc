...
#ifndef _MPBC
      const double *kpg2_ptr = basis.kpg2_PBC_ptr();
#else
      const double *kpg2_ptr = basis.kpg2_MPBC_ptr();
#endif

#ifdef _MPBC
#define PSDSTEPSIZE 0.5
#else
#define PSDSTEPSIZE 0.5
#endif
      ...
      if ( use_confinement )
      {
        ...
        {
          double e = PSDSTEPSIZE * ( kpg2_ptr[ig] + fstress[ig] );
          diag_[ispin][ikp][ig] = ( e < ecutpr ) ? PSDSTEPSIZE / ecutpr : PSDSTEPSIZE / e;
        }
      }
      else
      {
        for ( int ig = 0; ig < ngwloc; ig++ )
        {
          double e = PSDSTEPSIZE * kpg2_ptr[ig];
          diag_[ispin][ikp][ig] = ( e < ecutpr ) ? PSDSTEPSIZE / ecutpr : PSDSTEPSIZE / e;
        }
      }
    }
