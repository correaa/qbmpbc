...
extern void Write_WF_to_Matlab(boost::const_multi_array_ref<std::complex<double>, 3> const& wf, char const *name, int n);
#ifdef _USE_HDF5
 ...
 : s_(s), cd_(cd)
{
  ...
  for ( unsigned ispin = 0; ispin < wf.nspin(); ispin++ )
  {
    ...
    v_r[ispin].resize(vft->np012loc());
		#else
		v_r[ispin].resize(vft->np012());
		#endif
  }
	#ifndef _CHANGE_FT_CLASS
  tmp_r.resize(vft->np012loc());
	#else
	tmp_r.resize(vft->np012());
	#endif
...
void EnergyFunctional::update_vhxc(void)
{
  ...
  multi_array_ref<complex, 3> vXYZ (&vlocal_g[0], extents[np0][np1][np2]);
  multi_array_ref<complex, 3> vxyz (&tmp_r[0],    extents[np0][np1][np2]);

  plan v_backward(vXYZ, vxyz, FFTW_BACKWARD, FFTW_ESTIMATE);
  v_backward();
#endif

  // write out potential into file
  if(boost::filesystem::exists("print_pot"))
  {
    int np0=vft->np0();
    int np1=vft->np1();
    int np2=vft->np2();
    boost::multi_array_ref<std::complex<double>, 3> vion (&tmp_r[0], boost::extents[np0][np1][np2]);
    boost::multi_array<std::complex<double>, 3>     vhxc (boost::extents[np0][np1][np2]);
    std::complex<double> v;
    for ( unsigned i = 0; i < tmp_r.size(); i++ ) 
    { 
       v = v_r[0][i];
       *(vhxc.origin()+i) = v;
    }
    clog << "nspin = " << wf.nspin() << endl;
    clog << "np0,1,2 = " <<np0<<","<<np1<<","<<np2<< "size = "<<tmp_r.size()<<endl;
    clog << "Write potential into file " << endl;
#ifdef _USE_HDF5
//    Write_WF_to_HDF5(vbasis_,vion,"vion",0);
//    Write_WF_to_HDF5(vbasis_,vhxc,"vhxc",0);
#else
//    Write_WF_to_Matlab(vion,"vion",0);
//    Write_WF_to_Matlab(vhxc,"vhxc",0);
#endif
  }
              ...
              bool compute_stress, valarray<double>& sigma)
{
  ...
  for ( unsigned is = 0; is < fion.size(); is++ )
    for ( unsigned ia = 0; ia < fion[is].size(); ia++ )
      fion[is][ia] = 0.0;
...
#ifndef _MPBC
      const double *const kpg2  = wfbasis.kpg2_PBC_ptr();
#else
      const double *const kpg2  = wfbasis.kpg2_MPBC_ptr();
#endif
  ...
  clog<<"ekin_ = "<<ekin_<<" econf_ = "<<econf_<<" eps_ = "<<eps_<<" enl_ = "<<enl_<<endl;
  clog<<"  ecoul_ = "<<ecoul_<<" exc_ = "<<exc_<<" ets_ = "<<ets_<<" etotal_ = "<<etotal_<<endl;

  clog<<"ehart_ = "<<ehart_<<" esr_ = "<<esr_<<" eself_ = "<<eself_<<" ecoul_ = "<<ecoul_<<endl;
...
#ifndef _MPBC
        const double* kpg2 = wfbasis.kpg2_PBC_ptr();
#else
        SlaterDet& sd_var = *(wf.sd(ispin,ikp));
        const double* kpg2 = wfbasis.kpg2_MPBC_ptr();
//        const double* zhat2 = wfbasis.zhat2_ptr();
#endif
        const int ngwloc = wfbasis.localsize();
...
#else
        sd_var.anti_aliasing_mpbc(*ft[ikp]);
        sd.rs_mul_add_mpbc(*ft[ikp], &v_r[ispin][0], sdp);
#endif
      }
    }
    tmap["hpsi"].stop();
...
void EnergyFunctional::atoms_moved(void)
{
  ...
  sigma_esr = 0.0;
  for ( int is = 0; is < nsp_; is++ )
    for ( unsigned i = 0; i < fion_esr[is].size(); i++ )
      fion_esr[is][i] = 0.0;
...
void EnergyFunctional::cell_moved(void)
{
  ...
  vbasis_->resize(cell,s_.wf.refcell(),1.0*s_.wf.ecut());
#else
  ...
  /* Note: the non-local pseudopotential is using a smaller grid than the local pseudopotential */
#if 0
  // print non-local potentials to file
  std::ofstream ofs("twnl.dat");
  for ( int ig = 0; ig < nlp[0]->basis_.localsize(); ig++ )
  {
      const double * const g = nlp[0]->basis_.g_ptr();
      ofs<<ig<<" "<<g[ig]<<" "<< nlp[0]->twnl[0][ig]<<endl;
  }
  ofs.close();
#endif
