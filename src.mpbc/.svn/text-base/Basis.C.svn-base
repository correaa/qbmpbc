...
unsigned Basis::size() const { return size_; }
...
double Basis::kpg_PBC(int i) const   { return kpg_PBC_[i]; }
double Basis::gi(int i) const    { return gi_[i]; }
double Basis::kpgi_PBC(int i) const  { return kpgi_PBC_[i]; }
double Basis::g2(int i) const    { return g2_[i]; }
double Basis::kpg2(int i) const  { return kpg2_PBC_[i]; }
double Basis::kpg2_PBC(int i) const  { return kpg2_PBC_[i]; }
double Basis::g2i(int i) const   { return g2i_[i]; }
double Basis::kpg2i_PBC(int i) const { return kpg2i_PBC_[i]; }
...
#ifdef _MPBC
double Basis::kpg_MPBC(int i) const   { return kpg_MPBC_[i]; }
double Basis::kpgi_MPBC(int i) const  { return kpgi_MPBC_[i]; }
double Basis::kpg2_MPBC(int i) const  { return kpg2_MPBC_[i]; }
double Basis::kpg2i_MPBC(int i) const { return kpg2i_MPBC_[i]; }
double Basis::gzhat(int i) const { return gzhat_[i]; }
double Basis::zhat(int i) const  { return zhat_[i]; }
double Basis::zhat2(int i) const { return zhat2_[i]; }
#endif

const int*    Basis::idx_ptr(void) const   { return &(idx_[0]); }
const double* Basis::g_ptr(void)  const    { return &(g_[0]); }
const double* Basis::kpg_ptr(void) const{return &(kpg_PBC_[0]); }
const double* Basis::kpg_PBC_ptr(void)  const  { return &(kpg_PBC_[0]); }
const double* Basis::gi_ptr(void) const    { return &(gi_[0]); }
const double* Basis::kpgi_ptr(void) const  { return &(kpgi_PBC_[0]); }
const double* Basis::kpgi_PBC_ptr(void) const  { return &(kpgi_PBC_[0]); }
const double* Basis::g2_ptr(void) const    { return &(g2_[0]); }
const double* Basis::kpg2_ptr(void) const  { return &(kpg2_PBC_[0]); }
const double* Basis::kpg2_PBC_ptr(void) const  { return &(kpg2_PBC_[0]); }
const double* Basis::g2i_ptr(void) const   { return &(g2i_[0]); }
const double* Basis::kpg2i_PBC_ptr(void) const { return &(kpg2i_PBC_[0]); }
const double* Basis::gx_ptr(int j) const   { return &(gx_[j*localsize_[myrow_]]); }
const double* Basis::kpgx_ptr(int j) const { return &(kpgx_[j*localsize_[myrow_]]); }
#ifdef _MPBC
const double* Basis::kpg_MPBC_ptr(void)  const  { return &(kpg_MPBC_[0]); }
const double* Basis::kpgi_MPBC_ptr(void) const  { return &(kpgi_MPBC_[0]); }
const double* Basis::kpg2_MPBC_ptr(void) const  { return &(kpg2_MPBC_[0]); }
const double* Basis::kpg2i_MPBC_ptr(void) const { return &(kpg2i_MPBC_[0]); }
const double* Basis::gzhat_ptr(void) const { return &(gzhat_[0]); }
const double* Basis::zhat_ptr(void) const  { return &(zhat_[0]); }
const double* Basis::zhat2_ptr(void) const { return &(zhat2_[0]); }
const double* Basis::kpgx_xy_only_ptr(int j) const { return &(kpgx_xy_only_[j*localsize_[myrow_]]); }
const double* Basis::kpgx_zh_only_ptr(int j) const { return &(kpgx_zh_only_[j*localsize_[myrow_]]); }
#endif
...
boost::array<unsigned, 3u> Basis::np() const{return (boost::array<unsigned, 3u>){{np_[0],np_[1],np_[2]}};}
  ...
  double ecut)
{
  ...
  if ( ecut == 0.0 || cell.volume() == 0.0)
  {
    ...
    kpg_PBC_.resize(localsize_[myrow_]);
    gi_.resize(localsize_[myrow_]);
    kpgi_PBC_.resize(localsize_[myrow_]);
    g2_.resize(localsize_[myrow_]);
    kpg2_PBC_.resize(localsize_[myrow_]);
    g2i_.resize(localsize_[myrow_]);
    kpg2i_PBC_.resize(localsize_[myrow_]);
    gx_.resize(3*localsize_[myrow_]);
    kpgx_.resize(3*localsize_[myrow_]);
    isort_loc.resize(localsize_[myrow_]);
#ifdef _MPBC
    kpg_MPBC_.resize(localsize_[myrow_]);
    kpgi_MPBC_.resize(localsize_[myrow_]);
    kpg2_MPBC_.resize(localsize_[myrow_]);
    kpg2i_MPBC_.resize(localsize_[myrow_]);
    gzhat_.resize(localsize_[myrow_]);
    zhat_.resize(localsize_[myrow_]);
    zhat2_.resize(localsize_[myrow_]);
    kpgx_xy_only_.resize(3*localsize_[myrow_]);
    kpgx_zh_only_.resize(3*localsize_[myrow_]);
#endif
    return true;
  }
  ...
  else
  {
    ...
    nrods_ = 0;
    // rods (h,k,l)
    for ( int h = -np_[0]/2; h <= np_[0]/2-1; h++ )
    {
      for ( int k = -np_[1]/2; k <= np_[1]/2-1; k++ )
      {
        int lstart=lmax,lend=lmin;
        bool found = false;
        for ( int l = -np_[2]/2; l <= np_[2]/2-1; l++ )
        {
//          const double two_e = norm((kpx+h)*b0 + (kpy+k)*b1 + (kpz+l)*b2);
  ...
  // original qbox code, calc index according to rods
...
#else
  // index is fixed by convention
  idx_.resize(3*localsize_[myrow_]);
  int i = 0;
  // this is the order implied in FFTW 
  for ( int h = 0; h < np_[0]; h++ )
    for ( int k = 0; k < np_[1]; k++ )
      for ( int l = 0; l < np_[2]; l++ )
      {
        idx_[3*i]   = (h<np_[0]/2) ? h : (h-np_[0]);
        idx_[3*i+1] = (k<np_[1]/2) ? k : (k-np_[1]);
        idx_[3*i+2] = (l<np_[2]/2) ? l : (l-np_[2]);
        i++;
      }
  // this is the order in original Qbox (inconsistent with FFTW)
  //for ( int h = -np_[0]/2; h <= np_[0]/2-1; h++ )
  //  for ( int k = -np_[1]/2; k <= np_[1]/2-1; k++ )
  //    for ( int l = -np_[2]/2; l <= np_[2]/2-1; l++ )
  //    {
...
void Basis::update_g(void)
{
  ...
  for ( int i = 0; i < locsize; i++ )
  {
    ...
    /* g2 is used in computing v[i] and electron density */
    g2_[i] = norm(gt);
    kpg2_PBC_[i] = norm(kpgt);

    g_[i] = sqrt( g2_[i] );
    kpg_PBC_[i] = sqrt( kpg2_PBC_[i] );

    gi_[i] = g_[i] > 0.0 ? 1.0 / g_[i] : 0.0;
    kpgi_PBC_[i] = kpg_PBC_[i] > 0.0 ? 1.0 / kpg_PBC_[i] : 0.0;

    g2i_[i] = gi_[i] * gi_[i];
    kpg2i_PBC_[i] = kpgi_PBC_[i] * kpgi_PBC_[i];

#ifdef _MPBC
    /* Magnetic Periodic Boundary Condition (MPBC)
     * change to something else (only contains kinetic energy in x and z direction)
     * only orthogonal cell for now
     */
    int idx_hat = idx_[3*i+1]*np_[2] + ((idx_[3*i+2]>=0) ? idx_[3*i+2] : (idx_[3*i+2]+np_[2]));

#ifndef _SHORTFFT
    gzhat_[i] = idx_hat * cell_.b(2).z / np_[1];
    zhat_[i]  = idx_hat * cell_.b(1).y / np_[2];
#else
    gzhat_[i] = idx_hat * cell_.b(2).z / np_[1] * 2.0;
    zhat_[i]  = idx_hat * cell_.b(1).y / np_[2] * 1.0;
#endif

    kpg2_MPBC_[i] = kpgt.x*kpgt.x + gzhat_[i]*gzhat_[i];
    zhat2_[i] = zhat_[i]*zhat_[i];

    kpg_MPBC_[i] = sqrt( kpg2_MPBC_[i] );
    kpgi_MPBC_[i] = kpg_MPBC_[i] > 0.0 ? 1.0 / kpg_MPBC_[i] : 0.0;
    kpg2i_MPBC_[i] = kpgi_MPBC_[i] * kpgi_MPBC_[i];

    kpgx_xy_only_[i] = kpgt.x;
    kpgx_xy_only_[locsize+i] = zhat_[i]; //kpgt.y; //zhat_[i];
    kpgx_xy_only_[locsize+locsize+i] = 0.0;

    kpgx_zh_only_[i] = 0.0;
    kpgx_zh_only_[locsize+i] = 0.0;
    kpgx_zh_only_[locsize+locsize+i] = gzhat_[i]; //kpgt.z; //gzhat_[i];
#endif
...
void Basis::print(ostream& os)
{
  ...
  for ( int i = 0; i < localsize(); i++ )
  {
    ...
    os << setw(5) << i << "   "
       ...
       << setw(12) << setprecision(4) << 0.5 * kpg2_PBC(i)
       << endl;
  }
#else
  os << context().mype() << ": ";
  os << "   ig      i   j   k        gx      gy      gz       |k+g|^2      zhat       gzhat"
     << endl;
  os << context().mype() << ": ";
  os << "   --      -   -   -        --      --      --       -------      -------    -------"
     << endl;
  for ( int i = 0; i < localsize(); i++ )
  {
    ...
    os << setw(5) << i << "   "
       ...
       << setw(8) << setprecision(4) << gx(i+localsize())
       << setw(8) << setprecision(4) << gx(i+2*localsize())
       << setw(12) << setprecision(4) << 0.5 * kpg2_MPBC(i)
       << setw(12) << setprecision(4) << zhat(i)
       << setw(12) << setprecision(4) << gzhat(i)
       << endl;
  }
#endif
}
