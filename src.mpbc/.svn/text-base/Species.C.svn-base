...
////////////////////////////////////////////////////////////////////////////////
Species::Species(const Context& ctxt, string name) : 
	ctxt_(ctxt), 
	name_(name),
	uri_(""), 
	symbol_("Undef"), 
  ...
  if ( nquad_ == 0 && lmax_ > 0)
  {
    for ( unsigned l = 0; l <= lmax_; l++ )
    {
      ...
      if ( l != llocal_ )
      {
        ...
        for ( int i = 0; i < ndft_; i++ )
        {
          // wei: is not really vnl, it is vnl times phi (or xi)
...
void Species::phi(unsigned l, double r, double &val)
{
...
void Species::vpsr(unsigned l, double r, double &v)
{
  if ( l > lmax_ || r > rps_[ndft_-1] )
  {
    v = 0.0;
...
void Species::dvpsr(unsigned l, double r, double &v, double &dv)
{
  if ( l > lmax_ || r > rps_[ndft_-1] )
  {
    v = 0.0;
...
void Species::vnlg(unsigned l, double g, double &v)
{
  assert ( /*l >= 0 &&*/ l <= lmax_ );
...
void Species::dvnlg(unsigned l, double g, double &v, double &dv)
{
  assert ( /*l >= 0 && */ l <= lmax_ );
  if ( l == llocal_ || g > gspl_[ndft_-1] )
  {
...
ostream& operator << ( ostream &os, Species &s )
{
  ...
  else
  {
    ...
    os << setprecision(6);
    for ( unsigned l = 0; l <= s.lmax(); l++ )
    {
      const unsigned size = s.vps()[l].size();
