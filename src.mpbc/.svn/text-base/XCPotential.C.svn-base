...
ctxt_(cd.vcontext()), 
cd_(cd), 
vft_(*cd_.vft()), vbasis_(*cd_.vbasis())
{
...
//    int size = xcf_->np();
    ...
    if ( nspin_ == 1 )
    {
      for ( int j = 0; j < 3; j++ )
      {
        ...
        multi_array_ref<complex, 3> vxyz (&tmpr[0], extents[np0][np1][np2]);
        multi_array_ref<complex, 3> vXYZ (&tmp1[0], extents[np0][np1][np2]);
...
#else
        clog << "XCPotential::update() FFTW3 calls not implemented here!" << endl;
#endif
    ...
    if ( nspin_ == 1 )
    {
      for ( int j = 0; j < 3; j++ )
      {
        ...
        multi_array_ref<std::complex<double>, 3> vxyz (&tmpr[0], extents[np0][np1][np2]);
        multi_array_ref<std::complex<double>, 3> vXYZ (&tmp1[0], extents[np0][np1][np2]);
        plan v_forward (vxyz,vXYZ, FFTW_FORWARD,  FFTW_ESTIMATE);
        v_forward();

        /* normalization factor */
        double fac = 1.0/np012;
        for(int i=0; i<np012; i++) tmp1[i] *= fac;
#endif
        for ( int ig = 0; ig < ngloc_; ig++ )
        {
          // i*G_j*c(G)
