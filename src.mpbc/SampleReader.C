...
using namespace std;
typedef boost::array<unsigned, 3u> uarray3;
...
//  bool expandNamespaces = false;
  ...
  if ( ctxt_.onpe0() )
  {
    ...
    int event_int = event;
    ctxt_.ibcast_send(1,1,&event_int,1);
  } // onpe0
  else
  {
    // tasks > 0
    ...
    while ( !done )
    {
      ctxt_.ibcast_recv(1,1,&event_int,1,0,0);
      if ( event == end )
        done = true;
...
#else
          FourierTransform ft( (boost::array<unsigned, 3u>){{nx,ny,nz}} );
#endif
...
#else
            clog << "SampleReader::readSample (not from file) forward transform by FFTW3 not implemented yet" << endl;
            assert(0);
#endif
          }
        }
  ...
  if ( read_from_file )
  {
    ...
    if ( read_wf )
    {
      ...
      for ( unsigned ispin = 0; ispin < s.wf.nspin(); ispin++ )
      {
        for ( unsigned ikp = 0; ikp < s.wf.nkp(); ikp++ )
        {
          ...
          FourierTransform ft(basis,nx,ny,nz);
#else
          FourierTransform ft( (uarray3){{nx, ny, nz}} );
#endif
          ...
          unsigned wftmpr_size, wftmpr_block_size;
          if ( basis.real() )
          {
          ...
          for ( int nloc = 0; nloc < sd->nstloc(); nloc++ )
          {
           ...
           using boost::extents;
           using fftw3::plan;
           int np0=basis.np(0); int np1=basis.np(1); int np2=basis.np(2); 
	...
	int np012 = np0*np1*np2;
           ...
           // use FFTW3 transforms, by-passing qbox rods design
           multi_array_ref<complex, 3> wfXYZ (c.valptr(c.mloc()*nloc), extents[np0][np1][np2]);
           multi_array_ref<complex, 3>       wfxyz (&wftmp[0],                extents[np0][np1][np2]);

           /* normalization factor */
           for ( int i = 0; i < np012; i++ ) wftmp[i] /= (1.0*np012);

           plan wf_forward (wfxyz,wfXYZ, FFTW_FORWARD,  FFTW_ESTIMATE); wf_forward();
#else
           // FT from reciprocal space to intermediate space to real space in MPBC
           multi_array_ref<complex, 3> wfXYZ (c.valptr(c.mloc()*nloc),extents[np0][np1][np2]);
           multi_array_ref<complex, 2> wfXZh (c.valptr(c.mloc()*nloc),extents[np0][np1* np2]);
           multi_array<complex, 2>           wfXzh (                         extents[np0][np1* np2]);
           multi_array_ref<complex, 3>       wfXYz (wfXzh.origin(),          extents[np0][np1][np2]);
           multi_array_ref<complex, 3>       wfxyz (&wftmp[0],               extents[np0][np1][np2]);

           clog<<"wave function read.  transform to reciprocal space"<<endl;

           /* forward 2D FFT along x and y directions: wfxyz (wftmp) -> wfXYz (tmp) */
           /* 3D array, transform first two dimensions (+2) */
           //plan wf_xy_forward(+2, wfxyz,wfXYz, FFTW_FORWARD, FFTW_ESTIMATE);  wf_xy_forward();
           /* normalization factor */
           //for ( int i = 0; i < np012; i++ ) wfXzh.origin()[i] /= sqrt(np0*np1*np1*np2);
           /* forward 1D FFT along zhat direction: wfXYz = wfXzh (tmp) -> wfXZh (c) */
           /* 2D array, transform last one dimension (-1) */
           //plan wf_z_forward(-1, wfXzh,wfXZh, FFTW_FORWARD, FFTW_ESTIMATE);  wf_z_forward();

           ft.xy_forward_mpbc(wfxyz,wfXYz);
           ft.z_forward_mpbc (wfXzh,wfXZh);

           if(boost::filesystem::exists("print_wf_io"))
           {
               //Write_WF_to_Matlab(wfXYZ,"wf_rcpr_read",nloc);
               //Write_WF_to_Matlab(wfxyz,"wf_real_read",nloc);
           }
#endif //_MPBC
      ...
      for ( unsigned ispin = 0; ispin < s.wf.nspin(); ispin++ )
      {
        for ( unsigned ikp = 0; ikp < s.wf.nkp(); ikp++ )
        {
          ...
          for ( int nloc = 0; nloc < sd->nstloc(); nloc++ )
          {
            ...
            ft.forward(&wftmp[0],c.valptr(c.mloc()*nloc));
#else
            clog << "SampleReader::readSample Fourier transform of wfv by FFTW3 not implemented yet" << endl;
#endif
          }
        }
      }
