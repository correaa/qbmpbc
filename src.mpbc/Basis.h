...
#define _REGULARGDISTRIB
#endif
#endif

#ifdef _MPBC
#ifndef _CHANGE_FT_CLASS
#define _CHANGE_FT_CLASS
#endif
#endif
...
class Basis
{
  ...
  std::vector<double> kpg_PBC_;   // norm of g vectors g[localsize]
  std::vector<double> gi_;        // inverse norm of g vectors gi[localsize]
  std::vector<double> kpgi_PBC_;  // inverse norm of k+g vectors kpgi[localsize]
  std::vector<double> g2_;        // 2-norm of g vectors g2[localsize]
  std::vector<double> kpg2_PBC_;  // 2-norm of g vectors g2[localsize]
  std::vector<double> g2i_;       // inverse square norm of g vec g2i[localsize]
  std::vector<double> kpg2i_PBC_; // inverse square norm of k+g vec kpg2i[localsize]
...
#ifdef _MPBC
  std::vector<double> kpg_MPBC_;  // norm of g vectors g[localsize]
  std::vector<double> kpgi_MPBC_; // inverse norm of k+g vectors kpgi[localsize]
  std::vector<double> kpg2_MPBC_; // 2-norm of g vectors g2[localsize]
  std::vector<double> kpg2i_MPBC_;// inverse square norm of k+g vec kpg2i[localsize]
  std::vector<double> gzhat_;     // intermediate space variable
  std::vector<double> zhat_;      // intermediate space variable
  std::vector<double> zhat2_;     // intermediate space variable
  std::vector<double> kpgx_xy_only_;  // for magnetic translation (not working yet)
  std::vector<double> kpgx_zh_only_;  // for magnetic translation (not working yet)
#endif
  ...
  boost::array<unsigned, 3> np() const;
  ...
  unsigned size() const;              // total number of g vectors
  ...
  double kpg_PBC(int i) const;    // norm of k+g vectors kpg[i]
  double gi(int i) const;         // inverse norm of g vectors gi[i]
  double kpgi_PBC(int i) const;   // inverse norm of k+g vectors kpgi[i]
  double g2(int i) const;         // 2-norm of g vectors g2[i]
  double kpg2(int i) const;  // 2-norm of k+g vectors kpg[i]
  double kpg2_PBC(int i) const;   // 2-norm of k+g vectors kpg[i]
  double g2i(int i) const;        // inverse square norm of g g2i[i]
  double kpg2i_PBC(int i) const;  // inverse square norm of k+g kpg2i[i]
  double gx(int i) const;         // g vectors gx[i+localsize*j],j=0,1,2
  double kpgx(int i) const;       // k+g vectors kpgx[i+localsize*j],j=0,1,2
#ifdef _MPBC
  double kpg_MPBC(int i) const;   // norm of k+g vectors kpg[i]
  double kpgi_MPBC(int i) const;  // inverse norm of k+g vectors kpgi[i]
  double kpg2_MPBC(int i) const;  // 2-norm of k+g vectors kpg[i]
  double kpg2i_MPBC(int i) const; // inverse square norm of k+g kpg2i[i]
  double gzhat(int i) const; // intermediate space variable
  double zhat(int i) const;  // intermediate space variable
  double zhat2(int i) const; // intermediate space variable
#endif
  ...
  const double* kpg_PBC_ptr(void) const;
  const double* gi_ptr(void) const;
  const double* kpgi_ptr(void) const;
  const double* kpgi_PBC_ptr(void) const;
  const double* g2_ptr(void) const;
  const double* kpg2_ptr(void) const;
  const double* kpg2_PBC_ptr(void) const;
  const double* g2i_ptr(void) const;
  const double* kpg2i_PBC_ptr(void) const;
  const double* gx_ptr(int j) const;
  const double* kpgx_ptr(int j) const;
#ifdef _MPBC
  const double* kpg_MPBC_ptr(void) const;
  const double* kpgi_MPBC_ptr(void) const;
  const double* kpg2_MPBC_ptr(void) const;
  const double* kpg2i_MPBC_ptr(void) const;
  const double* gzhat_ptr(void) const; // intermediate space variable
  const double* zhat_ptr(void) const;  // intermediate space variable
  const double* zhat2_ptr(void) const; // intermediate space variable
  const double* kpgx_xy_only_ptr(int j) const;
  const double* kpgx_zh_only_ptr(int j) const;
#endif
