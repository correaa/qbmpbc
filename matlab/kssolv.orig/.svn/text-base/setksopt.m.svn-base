function options = ksoptset(varargin)
%
...
if (nargin == 0) && (nargout == 0)
    ...
    fprintf('                mixtype: [ anderson | broyden | pulay | pulay+kerker | none ]\n');
...
allfields = {'verbose';'maxscfiter';'maxdcmiter';'maxinerscf';'maxcgiter';...
             'scftol';'dcmtol';'cgtol';'mixtype';'mixdim';'betamix';...
             'brank';'X0';'rho0'};
...
switch field
    ...
    case {'scftol','dcmtol','cgtol','betamix'} % real scalar
        [validvalue, errmsg, errid] = nonNegReal(field,value);
    case {'maxscfiter','maxdcmiter','maxinerscf','maxcgiter','brank','mixdim'} % integer 
...
if ~valid
    ...
    errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be one of the following strings: anderson,broyden,broyden1,pulay,kerker,pulay+kerker',field);
else
    if (any(strcmp(value,...
       ...
       valid = 0;
       errid = 'MATLAB:funfun:setksopt:mixType';
       errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be one of the following: anderson, broyden, broyden1, pulay, kerker, pulay+kerker\n',field);
