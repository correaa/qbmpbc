...
%
% Perform 
%    1) the multiplication of two Wavefun objects
%              Z = X*Y;
%       which results in a matrix Z.
%    or
%    2) the multiplication of a Wavefun object and a matrix
%       (with a proper dimension). The result is a Wavefun 
%       object.
%              Z = X*M
% 
% usage: Z = X*Y;
...
if (nargin == 2)
   ...
   if ( isa(X,'Wavefun') )
      ...
      elseif (isnumeric(Y))
         ...
         error('The second operand must be either a Wavefun object');
         error('or an matrix');
      end;
   else
