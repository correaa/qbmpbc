...
figure(4);
h2_bond = 1.398397338;
Bxonline = zeros(Ny,1); Bxonline(:)=-Bx(Nx/2+1,:,Nz/2+1);
xgrid = [0:0.01:max(x)]; ygrid=interp1(x,Bxonline,xgrid,'spline');
xH1 = cell(2,2)/2 - h2_bond/2; BxH1 = interp1(x,Bxonline,xH1,'spline');
xH2 = cell(2,2)/2 + h2_bond/2; BxH2 = interp1(x,Bxonline,xH2,'spline');
plot(x,Bxonline,'.', xgrid,ygrid,'-', xH1,BxH1,'r+', xH2,BxH2,'r+');
xlabel('x');
ylabel('B_x');
Bind = BxH1; %max(max(-myBx));
