...
/* write wave function matrix into matlab file */
void Write_WF_to_Matlab(multi_array<complex<double>, 3>& wf,char *name,int n,int np0,int np1,int np2)
{
        ...
        fprintf(fp,"I = sqrt(-1);\n"); 
        fprintf(fp,"%s_%d = reshape(%s_%d_data(:,1)+%s_%d_data(:,2)*I, %d, %d, %d);\n",
        ...
        fprintf(fp,"%% %s_%d = reshape(%s_%d_data(:,1)+%s_%d_data(:,2)*I, %d, %d, %d);\n",
                   name,n,name,n,name,n,np0,np1,np2);
        for(int i=0; i<wf.shape()[0]; i++)
          for(int j=0; j<wf.shape()[1]; j++)
            for(int k=0; k<wf.shape()[2]; k++)
            {
  ...
  try
  {
    ...
    if ( c_.m() > 0 && c_.n() > 0 )
    {
           ...
           c_.m() != ctmp.m() && c_.n() == ctmp.n() )
      {
        ...
        cout << " SlaterDet::resize: grid: basis_->np0/1/2: "
             << basis_->np(0) << " " << basis_->np(1) << " " << basis_->np(2) << endl;
        cout << " SlaterDet::resize: grid: btmp.np0/1/2: "
             << btmp.np(0) << " " << btmp.np(1) << " " << btmp.np(2) << endl;
        ...
        FourierTransform ft2(*basis_,np0,np1,np2);  // ft2(*basis, basis_->np(0), basis_->np(1)
  ...
  const double* v, SlaterDet& sdp) const
{
  ...
  if ( basis_->real() )
  {
    /* In the current implementation, wave function has to be complex
     * this is done by removing the Gamma point (0,0,0) and add
     * (0.0001,0,0) in the input file
     */
    assert(0); // do not allow wave function to be real
  ...
  {
    // alf: iteration counter
    static int n_iter=0;
    n_iter++;
    // only one transform at a time
    int np0=ft.np0(); // not basis_->np(0);
    int np1=ft.np1(); // not basis_->np(1);
    int np2=ft.np2(); // not basis_->np(2);

    std::clog<<"np0 = "<<np0<<std::endl
             <<"np1 = "<<np1<<std::endl
             <<"np2 = "<<np2<<std::endl
             <<" np0*..*np1 = "<<np0*np1*np2<<std::endl
             <<"np012loc = "<<np012loc<<std::endl
             <<"mloc = "<<mloc<<std::endl;
    assert(np0*np1*np2==np012loc);

    if(boost::filesystem::exists("print_potential"))
    {
      std::clog<<"printing potential "<<std::endl;
      boost::multi_array_ref<double, 3> pot_array(const_cast<double*>(v), boost::extents[np0][np1][np2]);
      std::ofstream ofs("pot.list3.m");
      mathematica::oarchive moa(ofs);
      //moa<<pot_array;
      boost::filesystem::remove("print_potential");
    } 
    bool print_wf=false; if(boost::filesystem::exists("print_wf")){print_wf=true;}else{print_wf=false;}
    std::vector<double> normalization(nstloc(),0);
    std::vector<double> pot_en(nstloc(),0);
    std::vector<double> kin_en(nstloc(),0);
    std::vector<double> mag_en(nstloc(),0);
...
#ifdef _BFIELD
    /* assuming a cubic supercell */
    double B = (_BFIELD) * (2*M_PI*137./pow(length(basis_->cell().a(0)),2));
#else 
    double B = 0.0;
#endif

    clog<<"B = "<<B<<" mpbc is "<<(mpbc?"on":"off")<<endl;

    for(int n = 0; n < nstloc(); n++ )
    {
      // inverse FFT of c_ to real space assuming 3D PBC (which is not true for MPBC)
      ft.backward(c_.cvalptr(n*mloc),&tmp[0]);
      
      for(complex* i=&tmp[0]; i!= &tmp[0] + np012loc; ++i)
      { 
      // this is the normalization of tmp, but wait, 
      // before "forward" transform Qbox expects unnormalized tmp
	  	(*i)/=sqrt((double)np012loc);
      }

      // bind tmp with wfxyz, which is inverse FFT of c_ (assuming 3D PBC)
      multi_array_ref<complex, 3> wfxyz(&tmp[0], extents[np0][np1][np2]);

      static multi_array<complex, 3> wfXYZ(extents[np0][np1][np2]);
      static multi_array<complex, 3> wfxyz_tmp(extents[np0][np1][np2]);
      static multi_array<complex, 3> kx2_wfXYZ4sum(extents[np0][np1][np2]);
      static multi_array<complex, 3> ky2_wfXYZ4sum(extents[np0][np1][np2]);
      static multi_array<complex, 3> kz2_wfXYZ4sum(extents[np0][np1][np2]);
      static multi_array<complex, 3> Hkin_wfXYZ(extents[np0][np1][np2]);

      plan pF_wfxyz2wfXYZ(wfxyz, wfXYZ, FFTW_FORWARD, FFTW_ESTIMATE);
      plan pB_Hkin_wfXYZ2wfxyz_tmp(Hkin_wfXYZ, wfxyz_tmp, FFTW_BACKWARD, FFTW_ESTIMATE);

      // wfXYZ is the same as c_
      execute(pF_wfxyz2wfXYZ);

      // normalization of wfXYZ
      for(complex* i=wfXYZ.origin(); i!=wfXYZ.origin()+wfXYZ.num_elements(); ++i)
        (*i)/=sqrt((double)wfXYZ.num_elements());

      // magnetic field (B,0,0), vector potential A = (0,B*z,0)

      // apply kinetic energy along x-direction
      kin_en[n]+=apply_kx2(wfXYZ, kx2_wfXYZ4sum,length(basis_->cell().a(0)));

      // apply kinetic energy along y-direction
      if(B==0 and not mpbc)
      {
        kin_en[n]+=apply_ky2(wfXYZ, ky2_wfXYZ4sum,length(basis_->cell().a(0)));
      }
      else
      {
        if(not mpbc)
        {
          kin_en[n]+=apply_kymBz2(wfXYZ, ky2_wfXYZ4sum, length(basis_->cell().a(0)),B);
        }
        else
        {
          kin_en[n]+=apply_mpbc_kymBz2pkz2(wfXYZ, ky2_wfXYZ4sum, length(basis_->cell().a(0)),B);
        }
      }

      // apply kinetic energy along z-direction
      if(not mpbc)  // if mpbc (ky-Bz)^2 and kz^2 are computed together
        kin_en[n]+=apply_kz2(wfXYZ, kz2_wfXYZ4sum,length(basis_->cell().a(0)));
  
      set_zero(Hkin_wfXYZ);
      sum_to(Hkin_wfXYZ, kx2_wfXYZ4sum);
      sum_to(Hkin_wfXYZ, ky2_wfXYZ4sum);
      if(not mpbc)
          sum_to(Hkin_wfXYZ, kz2_wfXYZ4sum);

      // transform back to reciprocal space
      execute(pB_Hkin_wfXYZ2wfxyz_tmp);

      // normalization factor
      for(std::complex<double>* i=wfxyz_tmp.origin(); 
          i!=wfxyz_tmp.origin()+wfxyz_tmp.num_elements(); ++i)
      {
        *i/=sqrt((double)wfxyz_tmp.num_elements());
      }

      if(print_wf)
      { // alf: unless we use a flag, printing may start from any n --due to syncron--
        std::clog<<"printing wf "<<n<<std::endl;
        { 
          std::ofstream ofs(("wf."+boost::lexical_cast<std::string>(n)+".list3.m").c_str());
          {
            mathematica::oarchive moa(ofs);
            //moa<<wfxyz;
          }
        ...
        // caiwei: print out wave functions in Matlab format
        Write_WF_to_Matlab(wfXYZ,"wfXYZ",n,np0,np1,np2);

        boost::filesystem::remove("print_wf");
      }//if (print_wf)}

      pot_en[n]=0;
 
      //normalization[n]; //*=pow(length(basis_->cell().a(0)),6);
      //clog<<"after  "<<n<<" "<<normalization[n]<<endl;
      
      // alf: up to this tmp has the normalized wf
      for ( int i = 0; i < np012loc; i++ )
      { // normalization[n]+=norm(tmp[i]);
        pot_en[n]+=norm(tmp[i])*v[i]; //*pow(1./np0,3);
        tmp[i] *= v[i];
      }

      //pot_en[n]/=normalization[n];
      // adds laplacian
      //std::clog<<"laplacian added here L429"<<std::endl;
  
      for ( int i = 0; i < np012loc; i++ )
      {
        tmp[i] += wfxyz_tmp.origin()[i]; // alf: should move this up
        //tmp[i]=0; //comment to avoid application of hamiltonian
      }

      //fftw3::fftw_destroy_plan(p_b);
      //fftw3::fftw_destroy_plan(p_f);

      for(complex* i=&tmp[0]; i!= &tmp[0] + np012loc; ++i)
      { //this is the UNnormalization of tmp, but way before "forward" transform,
        // Qbox expects unnormalized tmp
        (*i)*=sqrt((double)np012loc); //note the *= , NOT /=, Qbox expects THAT normalization of tmp
      }
    ...
    // print results to screen
    {
       ...
       {
         //pot_en[n]/=normalization[n]; // alf: assuming cubic
         clog<<pot_en[n]<<" "; 
       }
       clog<<endl;
    }
    ...
    {
      clog<<"n_iter: "<<n_iter<<" etot_en: ";
      for(int n=0; n!=kin_en.size(); n++)
      {
        clog<< kin_en[n]+pot_en[n]+mag_en[n]<< " ";
      }
      clog<<endl;
    }
