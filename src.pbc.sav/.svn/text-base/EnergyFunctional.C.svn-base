...
#ifdef _EXTPOTENTIAL
  //add external potential to tmp_r
  //only works for orthogonal cell and serial run
  typedef std::complex<double> complex;
  boost::multi_array_ref<complex, 3> tmp_r_array(&tmp_r[0],
                         boost::extents[vft->np0()][vft->np1()][vft->np2()]);

  double x0, y0, z0, wx, wy, wz, V0;
  x0 = length(vbasis_->cell().a(0)) / 2.0; 
  y0 = length(vbasis_->cell().a(1)) / 2.0;
  z0 = length(vbasis_->cell().a(2)) / 2.0;
  wx = length(vbasis_->cell().a(0)) / 4.0; /* width of potential well in x */
  wy = length(vbasis_->cell().a(1)) / 4.0; /* width of potential well in y */
  wz = length(vbasis_->cell().a(2)) / 4.0; /* width of potential well in z */
  V0 = 0.5;                            /* strength of potential in au (1 au = 13.6 eV) */

  for (int i = 0; i < tmp_r_array.shape()[0]; i++ )
    for (int j = 0; j < tmp_r_array.shape()[1]; j++ )
      for (int k = 0; k < tmp_r_array.shape()[2]; k++ )
      {
          double x, y, z;
          x = i * length(vbasis_->cell().a(0)) / tmp_r_array.shape()[0]; 
          y = j * length(vbasis_->cell().a(1)) / tmp_r_array.shape()[1]; 
          z = k * length(vbasis_->cell().a(2)) / tmp_r_array.shape()[2]; 
         
          if ( (fabs(x-x0)>wx) || (fabs(y-y0)>wy)  || (fabs(z-z0)>wz) ) 
          {
             tmp_r_array[i][j][k] += V0;
          }
      }
#endif //_EXTPOTENTIAL
              ...
              bool compute_stress, valarray<double>& sigma)
{
  ...
  for ( int ispin = 0; ispin < wf.nspin(); ispin++ )
  {
    for ( int ikp = 0; ikp < wf.nkp(); ikp++ )
    {
      ...
      if ( use_confinement )
      {
      	assert(0); // confinement not implemented with magnetic field
  ...
  if ( compute_hpsi )
  {
    ...
    for ( int ispin = 0; ispin < wf.nspin(); ispin++ )
    {
      for ( int ikp = 0; ikp < wf.nkp(); ikp++ )
      {
        ...
        if ( use_confinement )
        {
          assert(0); // alf: not implemented with magnetic field
        ...
        {
          //std::clog<<"laplacian ignored here"<<std::endl;
          /*
          for ( int n = 0; n < sd.nstloc(); n++ )
          {
            for ( int ig = 0; ig < ngwloc; ig++ )
            {
              ...
              // alf: applies the laplacian operator in reciprocal space
            }
          }
          */
        }
