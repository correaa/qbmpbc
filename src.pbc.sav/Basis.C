  ...
  double ecut)
{
  ...
  // alf: I suspect here is where antialiasing is defined happens.
  ...
  // alf: this should be the Cube of gpoints
  // define safe enclosing domain for any k-point value in the BZ
  int hmax = (int) ( 0.5 + fac * ( length(defcell.a(0) ) ) );
  int hmin = - hmax;

  int kmax = (int) ( 0.5 + fac * ( length(defcell.a(1) ) ) );
  int kmin = - kmax;

  int lmax = (int) ( 0.5 + fac * ( length(defcell.a(2) ) ) );
  int lmin = - lmax;
  ...
  if ( real_ ) std::clog<<"basis is real at kp "<<kp<<std::endl;
  else std::clog<<"basis is complex at kp "<<kp<<std::endl;
  if ( real_ )
  {
    //assert(0); // alf: make sure basis are always complex
    ...
    for ( int h = 1; h <= hmax+1; h++ )
    {
      for ( int k = kmin-1; k <= kmax+1; k++ )
      {
        ...
        for ( int l = lmin-1; l <= lmax+1; l++ )
        {
          // alf: do not bother with this, this is for real basis only
  ...
  {
    // alf: this is the one we should care about in presence of magnetic field (with or without MPBC)
    ...
    for ( int h = hmin-1; h <= hmax+1; h++ )
    {
      for ( int k = kmin-1; k <= kmax+1; k++ )
      {
        ...
        for ( int l = lmin-1; l <= lmax+1; l++ )
        {
          ...
          if ( two_e < two_ecut )
          {
            lstart = lmin; //min(l,lstart);
            lend =   lmax; // max(l,lend);
            found = true;
          }
        }
        if ( found )
...
//#endif commented by alf
 ...
 // assert(lmin_used >= lmin);
  ...
  cout << "Basis::resize() np_012 = "<<np_[0]<<" "<<np_[1]<<" "<<np_[2]<<endl;
